...
Storage conventions (matching original notebooks):
  ...
  bounds.npy                – np.save / np.load, shape (3,2)  (stage 1)
...
# ---------------------------------------------------------------------------
def is_viable(g0: float, g1: float, g2: float) -> bool:
...
# Stage 1 – prior KDE
# ===========================================================================
def stage_prior_kde(run_dir: str, n_samples: int) -> tuple:
    """
    Rejection-sample n_samples viable EOS triples, fit a KDE, derive
    per-parameter bounds from the sample min/max.

    Saves
    -----
    viable_gamma_samples.npy  shape (n_samples, 3)
    bounds.npy                shape (3, 2)  – [[lo0,hi0],[lo1,hi1],[lo2,hi2]]
    """
    samples_path = os.path.join(run_dir, "viable_gamma_samples.npy")
    bounds_path  = os.path.join(run_dir, "bounds.npy")

    print(f"\n[prior_kde] Rejection-sampling {n_samples} viable EOS triples ...")
    viable  = []
    n_total = 0
    ...
    viable = np.array(viable)                      # (n_samples, 3)
    print(f"[prior_kde] Acceptance rate: {len(viable) / n_total:.3%}")

    np.save(samples_path, viable)
    print(f"[prior_kde] Saved viable samples -> {samples_path}")

    # Bounds: per-parameter min/max of the full viable sample array.
    # The rejection sweep covers [1,5]^3 >> any realistic EOS posterior,
    # so these bounds conservatively envelope the prior support.
    bounds = np.column_stack([viable.min(axis=0), viable.max(axis=0)])  # (3, 2)
    np.save(bounds_path, bounds)
    print(f"[prior_kde] Derived bounds (g0, g1, g2):")
...
def stage_hierarchical(
    ...
    # bounds rows: [g0, g1, g2]; columns: [lo, hi]
    bounds_dict = {
    ...
    # Seed walkers from KDE, requiring viability
    lo = bounds[:, 0]
    hi = bounds[:, 1]
    init_pos = []
...
def stage_ppd(run_dir: str, result, n_ppd_samples: int = 200, n_mass_points: int = 200):
    ...
    for _, row in draws.iterrows():
        family = _build_family(
            row.param1, LOG10_P1_CGS,
            row.param2, LOG10_P2_CGS,
...
def main():
    ...
    if "prior_kde" in stages:
        if args.force or not os.path.exists(samples_path):
            kde, bounds = stage_prior_kde(run_dir, args.n_kde_samples)
        else:
