...
def _build_family(p1, lp1_cgs, p2, lp2_cgs, p3, causal=0, param_bounds=None):
    """
    Validate EOS parameters and return the LALSim NS family object.

    Steps
    -----
    0. Hard prior-bounds check (cheap; prevents KDE tails reaching LALSim).
    1. Ordering constraint on dividing pressures.
    2. Viability pre-filter (cheap).
    3. Build piecewise-polytrope EOS object (analytic, fast).
    4. Integrate TOV equations to get mass-radius-tidal family (expensive, ~10 ms).
    5. Speed-of-sound causality check.
    ...
    param_bounds : dict or None
        If provided, must contain keys 'param1', 'param2', 'param3',
        'log10_p1', 'log10_p2', each mapping to a (lo, hi) tuple.
        Parameters outside these bounds are rejected before reaching LALSim.

    Returns
    -------
    family : LALSim family object, or None if EOS is unphysical.
    """
    # Step 0: hard bounds check — keeps KDE tails out of LALSim entirely
    if param_bounds is not None:
        lo1, hi1 = param_bounds["param1"]
        lo2, hi2 = param_bounds["param2"]
        lo3, hi3 = param_bounds["param3"]
        lop1_lo, lop1_hi = param_bounds["log10_p1"]
        lop2_lo, lop2_hi = param_bounds["log10_p2"]
        if not (lo1 <= p1 <= hi1 and lo2 <= p2 <= hi2 and lo3 <= p3 <= hi3):
            return None
        if not (lop1_lo <= lp1_cgs <= lop1_hi and lop2_lo <= lp2_cgs <= lop2_hi):
            return None

    # Step 1: ordering constraint
    if lp1_cgs >= lp2_cgs:
        return None
    ...
    try:
        ...
        # LALSim / GSL can raise on pathological parameter combinations
        # (e.g. non-monotone pressure grid) that slip past the viability
        # pre-filter.  Treat these as unphysical.
        return None
...
class EOSHyperparameterLikelihood(Likelihood):
    ...
    Sampled parameters (passed via self.parameters at likelihood evaluation)
    -------------------------------------------------------------------------
    param1, param2, param3 : float
        Adiabatic indices Γ₀, Γ₁, Γ₂.
    log10_p1, log10_p2 : float
        Dividing pressures in log10(dyn cm⁻²) (CGS), with log10_p1 < log10_p2
        enforced by the prior (and additionally guarded in _build_family).
    """
    ...
    def __init__(
        ...
        self._param_bounds = param_bounds  # dict of {name: (lo, hi)} or None
    ...
    def log_likelihood(self, parameters=None):
        ...
        family = _build_family(
            parameters["param1"], parameters["log10_p1"],
            parameters["param2"], parameters["log10_p2"],
